...
public class inmueblesDAO {
    ...
    public void insertarRegistro(inmuebles inmueble) {
        ...
        try {
            ...
            ps.setInt(10, inmueble.getPrecio());
            ps.setString(11, inmueble.getUbicacion());
            ps.setString(12, String.valueOf(inmueble.getObservaciones()));
    ...
    public inmuebles seleccionarRegistro(inmuebles inmueble){
        ...
        try {
            ...
            if (!rs.next())
                ...
                return new inmuebles(rs.getInt("ID_Inmueble"), rs.getInt("Tipo_Inmueble"), rs.getInt("Tipo_Operacion "), rs.getInt("Num_Recamaras"), rs.getInt("Num_Banos"), rs.getInt("Num_MedBanos"), rs.getInt("Num_Garage"), rs.getBoolean("Jardin"), rs.getBoolean("Alberca"), rs.getInt("Precio"), rs.getString("Ubicacion"), rs.getString("Observaciones"));
            con.close(rs);
            con.close(connection);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void actualizarRegistro(inmuebles inmueble){
        ...
        try {
            sql = "UPDATE inmuebles "+
                    "SET ID_Inmueble = ?, Tipo_Inmueble = ?, Tipo_Operacion = ?, Num_Recamaras = ?, Num_Banos = ?, Num_MedBanos = ?, Num_Garage = ?, Jardin  = ?, Alberca = ?, Precio = ?, Ubicacion = ?, Observaciones = ? "+
